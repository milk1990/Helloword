...
import com.example.mystructs.app.book.ScrollOverListView.OnScrollOverListener;
...
public class PullDownView extends LinearLayout implements OnScrollOverListener {
    ...
    private static final int WHAT_NONE_DATA=6; // Handler what 已经取完所有的数据
	
	private static final int DEFAULT_HEADER_VIEW_HEIGHT = 85; // 头部文件原本的高度
	...
	private void initHeaderViewAndFooterViewAndListView(Context context){
		...
		 * 如果要修改，和它相关的设置都要更改
		 */
		mHeaderView = LayoutInflater.from(context).inflate(R.layout.pulldown_head, null);
		...
		 * 自定义脚部文件
		 */
		mFooterView = LayoutInflater.from(context).inflate(R.layout.pulldown_foot, null);
	...
	/**
	     * 通知已经没有数据了，要放在Adapter.notifyDataSetChanged后面
	     * 当你执行完更多任务之后，调用这个notyfyDidMore()
	     * 才会隐藏加载圈等操作
	     */
	public void notifyNoneData() {
	    mUIHandler.sendEmptyMessage(WHAT_NONE_DATA);
	}
	/**
	     * 设置监听器
	     * @param listener
	...
	private Handler mUIHandler = new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			case WHAT_NONE_DATA :{
				mIsFetchMoreing = false;
				mFooterTextView.setText("暂无其他的数据");
				mFooterLoadingView.setVisibility(View.GONE);
			  }
			}
		}
