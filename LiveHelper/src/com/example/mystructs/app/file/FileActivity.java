...
import com.example.mystructs.app.cart.CartAddActivity;
...
public class FileActivity extends Activity implements OnPullDownListener,OnItemClickListener {
	private static final int WHAT_DID_LOAD_DATA = 0;
	private static final int WHAT_DID_REFRESH = 1;
	private static final int WHAT_DID_MORE = 2;
	private ListView mListView;
    private FileItemAdapter bAdapter;
    private List<Map<String, Object>> maps=new ArrayList<Map<String,Object>>();
    private ProgressBarDialog progressDialog = null;  
    private Cursor cursor;
    private  String sql="select "+DB._ID+","+DB.Bill_Column1+","+DB.Bill_Column2+" from "+DB.Table_Name3+" order by "+DB.Bill_Column2+" desc";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		bAdapter=new FileItemAdapter(this, maps);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				while (!cursor.isAfterLast()) {
					...
					map.put(DB.Bill_Column1,cursor.getString(1));
					map.put(DB.Bill_Column2,cursor.getString(2));
					maps.add(map);
					cursor.moveToNext();
				}
	...
	public void onItemClick(AdapterView<?> parent,View view,final int position,long id){
		new AlertDialog.Builder(this).setTitle("功能表").setItems(new String[]{"详细","删除"}, new OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				case 0:
					...
					intent.putExtra("id", (Integer)maps.get(position).get("id"));
					startActivity(intent);
					break;
				case 1:
					int i=deleteItem(" _id ="+maps.get(position).get("id").toString());
					if (i>0) {
						MyToast.shortToast(FileActivity.this, "删除成功");
						startProgressDialog();
						maps.clear();
						loadData(sql);
					}else {
						MyToast.shortToast(FileActivity.this, "删除失败");
					}
					break;
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case Menu.FIRST+1:
			Intent intent=new Intent();
		    intent.setClass(FileActivity.this, CartAddActivity.class);
	...
	private int deleteItem(String arg0){
		int i=this.getContentResolver().delete(Constant.CONTENT_UTI_Bill, arg0, null);
		return i;
	}
}